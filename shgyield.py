...
RMpav = (Rlbp * np.exp(1j * delta/2))/(1 + (Rvlp * Rlbp * np.exp(1j * delta)))\
        * np.sin(delta/2)/(delta/2)
RMsav = (Rlbs * np.exp(1j * delta/2))/(1 + (Rvls * Rlbs * np.exp(1j * delta)))\
        * np.sin(delta/2)/(delta/2)
#rMp = rlbp # neglect multiple reflections for 1w
#RMp = Rlbp # neglect multiple reflections for 2w
