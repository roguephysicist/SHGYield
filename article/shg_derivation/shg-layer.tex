...
both gauges give, as they should, the same result. We discuss a possible
numerical check up on this equivalency. Also, the so called
three-layer-model for the calculation of the surface radiated SH efficiency
is presented.
...
found in
Ref. \cite{sipe00} and \cite{lambrecht00}.
...
\end{equation}
where $H_0=p^2/2m + V(\bfr)$, where $V(\bfr)=V(\bfr + \bfR)$ is the periodic
crystal potential,
with $\bfR$ the real-space lattice vector.
 The
...
\int_{\Omega}d^{3}r\, u_{n\mathbf{k}}^{*}(\mathbf{r})u_{m\mathbf{q}}(\mathbf{r})=\delta_{nm}\delta_{\mathbf{\mathbf{k},q}}
,
\end{equation}
...
we use the following valid result for any periodic
...
,
\end{equation}
with $\hat\bfv$ the velocity operator. Neglecting nonlocal potentials
in $\hat{H}_0$ we obtain, on one hand
\begin{equation}\label{conhr}
[\hat\bfr,\hat{H}_0]=i\hbar\frac{\hat\bfp}{m_e}
,
\end{equation}
with $\hat\bfp$ the momentum operator, with $m_e$ the mass of the
electron. On the other hand,
\begin{equation}\label{conhrnm}
\bra{n\bfk}
[\hat\bfr,\hat{H}_0]
\ket{m\bfk}=\bra{n\bfk}\hat\bfr\hat{H}_0-\hat{H}_0\hat\bfr\ket{m\bfk}
=(\hbar\go_m(\bfk)-\hbar\go_n(\bfk))\bra{n\bfk}\hat\bfr\ket{m\bfk}
,
\end{equation}
thus defining $\go_{nm\bfk}=\go_n(\bfk)-\go_m(\bfk)$ we get
\begin{equation}\label{pmnrmn}
\bfr_{nm}(\bfk)=\frac{\bfp_{nm}(\bfk)}{im_e\go_{nm}(\bfk)}
=\frac{\bfv_{nm}(\bfk)}{i\go_{nm}(\bfk)}
\quad\quad n\ne m
.
\end{equation}
...
\begin{equation}\label{traza}
  \calo=Tr(\hat\calo\hat\rho)=Tr(\hat\rho\hat\calo),
\end{equation}
where $Tr$ is the trace, that as we have shown has the property of
being 
...
with below,
and $\mathrm{Tr}$ denotes the trace. We define
...
\psi_{n\mathbf{k}}(\mathbf{r}),\label{pofs2} \\
&=&\frac{1}{m}\int d^3 r
...
two terms, one coming from the first term of Eq. (\ref{rho2}) and the other
from the second term of the same equation. Then we have, after
...
and that for SHG $\go_1=\go_2=\go$ and $\go_3=2\go$.
...
calculate the surface SHG radiated from a semiconductor within the dipole
approximation, and showed how to calculate the layer-by-layer
contribution to the optical signal.
We derived the nonlinear surface susceptibility
tensor $\boldsymbol{\chi}$ within the longitudinal gauge and thus we
decomposed $\boldsymbol{\chi}$ 
into intraband and interband one-electron transitions. 
%Also,
...
\appendix
\section{Expressions for $\chi^s_{\rma\rmb\rmc}$}
...
.
\end{eqnarray}  
Finally, following Ref.~\onlinecite{nastosPRB05} we simply change
...
We should check if these results are equivalent to Eqs. 40 and 41 of
Cabellos et. al. \cite{cabellosPRB09} for a bulk system for which we
